 ...
 * Route the BTDF entirely into the specular bucket for simplicity.
 */

// See 5.9 Microfacet Transmission
// See 5.12 BSDF Evaluation
BsdfSplit EvaluateBtdfSplit(vec3 wi, Ray ray, SurfaceInteraction si) {
    Material mat = si.mat;
    vec3 ns = si.ns;
    vec3 wo = -ray.d;
    float eta_i = si.eta_i;
...
// See 5.9 Microfacet Transmission
// See 5.12 BSDF Evaluation
BsdfSplit EvaluateBtdfAndPdfSplit(vec3 wi, Ray ray, SurfaceInteraction si, out float btdfPdf) {
    Material mat = si.mat;
    vec3 ns = si.ns;
    vec3 wo = -ray.d;
    float eta_i = si.eta_i;
