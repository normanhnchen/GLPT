...
// See 5.12 BSDF Evaluation
BsdfSplit EvaluateBrdfSplit(vec3 wi, Ray ray, SurfaceInteraction si) {
    Material mat = si.mat;
    ...
    float cosTheta = max(nsDotWi, 0.0);
    BsdfSplit bsdfSplit;
    bsdfSplit.diffuse = diffuse * cosTheta;
    bsdfSplit.specular = specular * cosTheta;

    return bsdfSplit;
}

// See 5.12 BSDF Evaluation
BsdfSplit EvaluateBrdfAndPdfSplit(vec3 wi, Ray ray, SurfaceInteraction si, out float brdfPdf) {
    Material mat = si.mat;
